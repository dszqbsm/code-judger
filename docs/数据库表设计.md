...
- **时间戳**: 统一使用 `TIMESTAMP`，包含 `created_at` 和 `updated_at`
- **软删除**: 使用 `deleted_at` 字段标记删除状态
- **枚举值**: 使用 `ENUM` 类型限制取值范围

## 2. 用户模块数据表

### 2.1 用户基础信息表 (users)

该表存储系统中所有用户的基础信息，包括认证凭据和角色权限。
...
**字段说明**:
- `password_hash`: 使用bcrypt算法，成本因子为12
- `role`: 三级权限模型，学生 < 教师 < 管理员
- `status`: 支持账户禁用和封禁功能
- `last_login_ip`: 支持IPv6地址格式

### 2.2 用户令牌表 (user_tokens)

该表存储用户的JWT刷新令牌，实现令牌黑名单机制。
...
### 2.3 用户登录日志表 (user_login_logs)

该表记录用户的登录行为，用于安全审计和异常检测。
...
### 2.4 用户统计信息表 (user_statistics)

该表存储用户的统计信息，支持排行榜和数据分析功能。
...
('admin', 'admin@oj.local', '$2a$12$rGKqNQR4P8gG5QJ0QJ0QJeJ4X4X4X4X4X4X4X4X4X4X4X4X4X4X4X4', '系统管理员', 'admin', 'active', true);
